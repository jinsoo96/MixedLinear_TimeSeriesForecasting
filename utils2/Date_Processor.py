...
class Preprocessor:
    ...
    def apply_pca(self, threshold=0.80):
        """PCA 적용 (기본값: 80% 이상 분산)"""
        pca = PCA()
        df_pca = pca.fit_transform(self.df)
        total_variance = np.cumsum(pca.explained_variance_ratio_)
        n_over_threshold = len(total_variance[total_variance >= threshold])
        df_pca = df_pca[:, :n_over_threshold] 
        self.df = pd.DataFrame(df_pca, columns=[f'PC{i+1}' for i in range(n_over_threshold)])
...
class WeightedAverageCalculator:
    ...
    def __init__(self, weight_file, data_file, drop_column):
        ...
        self.weights = self.weight_df.drop(columns=[drop_column]).mean()
...
def apply_pca(data_frame, threshold=0.80):
    """
    PCA 적용 함수
    
    Args:
        data_frame: 입력 데이터프레임
        threshold: 분산 설명 비율 임계값 (기본값: 0.80)
    
    Returns:
        pd.DataFrame: PCA 적용된 데이터프레임
    """
    pca = PCA()
    ...
    n_over_threshold = len(total_variance[total_variance >= threshold])
    df_pca = df_pca[:, :n_over_threshold] 
    result_df = pd.DataFrame(df_pca, columns=[f'PC{i+1}' for i in range(n_over_threshold)])
...
def error_cal(Actual_data, predicted_data, method='mape', return_df=False):
    ...
    Args:
        Actual_data: 실제 데이터
        predicted_data: 예측 데이터
        method: 오차 계산 방법 ('mape', 'mse', 'rmse')
        return_df: 데이터프레임 반환 여부
    
    Returns:
        오차 값 또는 (오차 값, 데이터프레임) 튜플
    """
